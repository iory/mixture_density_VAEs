...
def get_file_name(expDir, vaeParams, trainParams):
    ...
    results_file_name = pjoin(expDir, "train_logs/dlgmm_trainResults"+output_file_base_name+".txt")
    file_exists_counter = 0
    while os.path.isfile(results_file_name):
        file_exists_counter += 1
        results_file_name = pjoin(expDir, "train_logs/dlgmm_trainResults"+output_file_base_name+"_"+str(file_exists_counter)+".txt")
...
### Training function
def trainDLGMM(data, vae_hyperParams, hyperParams, param_save_path, logFile=None):
...
if __name__ == "__main__":
    ...
    logging_file = open(inArgs.experimentDir+"train_logs/dlgmm_trainResults"+outfile_base_name+".txt", 'w')
    param_file_name = inArgs.experimentDir+"params/dlgmm_params"+outfile_base_name+".ckpt"
    ...
    #sample_from_model(model, param_file_name, vae_hyperParams, inArgs.experimentDir+'samples/dlgmm_samples'+outfile_base_name)
